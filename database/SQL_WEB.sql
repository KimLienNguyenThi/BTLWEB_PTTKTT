...
CREATE  TABLE ChiTietPTL (
	...
	--Soluongtl INT,
	--MaPTL INT,
	--FOREIGN KEY (MaSach) REFERENCES khosachthanhly (MaSach),
	--FOREIGN KEY (MaPTL) REFERENCES PhieuThanhLy (MaPTL),
	--constraint ChiTietPTL_MaPTL_MaSach PRIMARY KEY (MaPTL, MaSach)
    MaPTL INT,
	MaSachkho INT,
	 Soluongtl INT,
    FOREIGN KEY (MaPTL) REFERENCES PhieuThanhLy (MaPTL),
...
--RANG BUOC PHIEU TRA
CREATE or ALTER TRIGGER CheckNgayTraPhieuTra
ON PhieuTra
AFTER INSERT
AS
BEGIN
   ...
   IF EXISTS (
        SELECT 1
        FROM inserted AS i
        JOIN PhieuMuon AS pm ON i.MaThe = pm.MaTHE
        WHERE i.NgayTra <= pm.NgayMuon

    )
    BEGIN
	   RAISERROR('Dữ liệu KHONG thành công',16,1)
       ROLLBACK TRANSACTION; 
    END
	END;
	
--insert into PhieuTra ( NgayTra, MaNV, MaThe, TongSachTra) values ( '2024-10-3', 2, 1, 3);
--insert into PhieuTra ( NgayTra, MaNV, MaThe, TongSachTra) values ( '2022-10-3', 2, 1, 3);
...
AS
	SELECT PN.MAPN, NgayNhap, MaNV, MaNCC, SUM(ABS(SACH.GIASACH*SOLUONGNHAP)) AS N'TỔNG TIỀN', SUM(SOLUONGNHAP) AS N'TỔNG SÁCH'
